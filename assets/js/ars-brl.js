...
btngo.addEventListener('click', async (event) => {
    ...
    code.textContent = result.ARSBRL.code
    codein.textContent = result.ARSBRL.codein
    nome.textContent = result.ARSBRL.name
    high.textContent = result.ARSBRL.high
    low.textContent = result.ARSBRL.low
    bid.textContent = result.ARSBRL.bid
    date.textContent = result.ARSBRL.create_date

    let converter;
    converter = parseFloat(result.ARSBRL.bid);
    converter = 1 / converter;
    converter = converter.toFixed(2)
    converte.textContent = converter

    venda = result.ARSBRL.bid
